...
def imprime_botoes(img, posicao, letra, tamanho = 35, cor_retangulo = BRANCO):
    ...
    cv2.putText(img, letra, (posicao[0]+5,posicao[1]+30), cv2.FONT_HERSHEY_COMPLEX, 1, PRETO, 2)
    return img
...
while True:
    ...
    if len(todas_maos) == 1:
        ...
        if todas_maos[0]['lado'] == 'Left':
            ...
            for indice_linha, linha_teclado in enumerate(teclas):
                for indice, letra in enumerate(linha_teclado):
                    if sum(info_dedos_mao1) <= 1:
                        letra = letra.lower()
                    img = imprime_botoes(img, (offset+indice*55, offset+indice_linha*55),letra)
                    if offset+indice*55 < indicador_x < 85+indice*55 and offset+indice_linha*55<indicador_y<85+indice_linha*55:
                        img = imprime_botoes(img, (offset+indice*55, offset+indice_linha*55),letra, cor_retangulo=VERDE)
                        if indicador_z < -85:
                            contador = 1
                            escreve = letra
                            img = imprime_botoes(img, (offset+indice*55, offset+indice_linha*55),letra, cor_retangulo=AZUL_CLARO)
            if contador:
                contador += 1
                if contador == 3:
                    texto+= escreve
            ...
            cv2.rectangle(img, (offset, 390), (500, 350), BRANCO, cv2.FILLED)
            cv2.rectangle(img, (offset, 390), (500, 350), AZUL, 1)
            cv2.putText(img, texto[-40:], (offset, 380), cv2.FONT_HERSHEY_COMPLEX, 1, PRETO, 2)
